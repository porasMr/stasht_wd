...
class EditMemoryScreen extends StatefulWidget {
  ...
  @override
  State<EditMemoryScreen> createState() => _EditMemoryScreenScreenState();
}

class _EditMemoryScreenScreenState extends State<EditMemoryScreen>
...
String photoId="";
  ...
  void initState() {
    ...
    PrefUtils.instance.getUserFromPrefs().then((value) {
      ...
      setState(() {});
          ...
          photoGroupModel=CommonWidgets.groupGalleryPhotosByDate(widget.photosList,widget.future);
    ...
    PrefUtils.instance.getInstaPrefs().then((value) {
      ...
      instaModel = value;
      instaGroupModel = CommonWidgets.groupPhotosForFBAndINSTAByDate(instaModel);
            viewRefershOtherTab();
  ...
  selectionOfAllPhoto() {
    ...
    for (int p = 0; p < widget.memoryListData!.length; p++) {
      if (widget.memoryListData![p].type == "image") {
        updateSelectedValue2(widget.memoryListData![p].typeId!);
  ...
  void updateSelectedValue2(String selectedId) {
    ...
    for (int j = 0; j < photoGroupModel[k].photos.length; j++) {
      if (photoGroupModel[k].photos[j].assetEntity.id == selectedId) {
        photoGroupModel[k].photos[j].selectedValue = true;
        photoGroupModel[k].photos[j].isEditmemory = true;
      }
    }
    }
      ...
      appBar: AppBar(
        ...
        title: Row(
          children: [
            ...
            Text(
              widget.isAddPhoto! ? "Edit Memory" : "Add Media",
              style: appTextStyle(
                  fz: 22,
                  height: 28 / 22,
                  fm: robotoRegular,
        ...
        children: [
          widget.isAddPhoto!
              ? Column(
                  ...
                  children: [
                    SizedBox(
                      height: widget.fromEdit == true ? 0 : 8,
                    ),
                    const Divider(
                                ...
                                Expanded(
                                  child: TextFormField(
                                    ...
                                    decoration: InputDecoration(
                                      ...
                                      hintStyle: appTextStyle(
                                        fz: isTitleFocused ? 14 : 21,
                                        color: selectLabel == "Choose a Lable"
                                            ? AppColors.hintColor
                                            : Colors.black,
                                        fm: robotoRegular,
                                      ),
...
allPhotoGroupModel,        viewRefersh,

      );
    } else if (selectedIndex == 1) {
      return CommonWidgets.albumView(
photoGroupModel,        viewRefershOtherTab,
...
viewRefersh();
    ...
    for (int i = 0; i < allPhotoGroupModel.length; i++) {
      for (int p = 0; p < allPhotoGroupModel[i].photos.length; p++) {
        if (allPhotoGroupModel[i].photos[p].type == "image") {
          newUpdateSelectedValue2(allPhotoGroupModel[i].photos[p].id!,allPhotoGroupModel[i].photos[p].isSelected);
        } else if (allPhotoGroupModel[i].photos[p].type == "insta") {
          newUpdateInstaSelectedValue(
          ...
          newUpdateDriveSelectedValue(allPhotoGroupModel[i].photos[p].id!.toString(),allPhotoGroupModel[i].photos[p].isSelected);
        } else if (allPhotoGroupModel[i].photos[p].type == "drive") {
          newUpdateSelectedValue(
              allPhotoGroupModel[i].photos[p].id!.toString(),allPhotoGroupModel[i].photos[p].isSelected);
        }
      }
    }
 ...
 void newUpdateSelectedValue(String selectedId,bool selectedValue) {
    for (int i = 0; i < photoGroupModel.length; i++) {
      for (int j = 0; j < photoGroupModel[i].photos.length; j++) {
        if (photoGroupModel[i].photos[j].assetEntity.id == selectedId) {
  ...
  void newUpdateSelectedValue2(String selectedId,bool selectedValue) {
    for (int k = 0; k < photoGroupModel.length; k++) {
      for (int j = 0; j < photoGroupModel[k].photos.length; j++) {
        if (photoGroupModel[k].photos[j].assetEntity.id == selectedId) {
  ...
  void newUpdateFbSelectedValue(String selectedId,bool selectedValue) {
    print(selectedId);
    for (int j = 0; j < fbGroupModel.length; j++) {
      for (int i = 0; i < fbGroupModel[j].photos.length; i++) {
        if (fbGroupModel[j].photos[i].id == selectedId) {
  ...
  void newUpdateInstaSelectedValue(String selectedId,bool selectedValue) {
    for (int j = 0; j < instaGroupModel.length; j++) {
      for (int i = 0; i < instaGroupModel[j].photos.length; i++) {
        if (instaGroupModel[j].photos[i].id == selectedId) {
  ...
  void newUpdateDriveSelectedValue(String selectedId,bool selectedValue) {
    for (int j = 0; j < driveGroupModel.length; j++) {
      for (int i = 0; i < driveGroupModel[j].photos.length; i++) {
        if (driveGroupModel[j].photos[i].id == selectedId) {
      ...
      if (categoryMemoryModelWithoutPage.subCategories!.isNotEmpty) {
          ...
          if (categoryMemoryModelWithoutPage.subCategories != null) {
            for (var subCategory
              ...
              if (widget.subId != "") {
                categoryMemoryModelWithoutPage.subCategories![0].isselected =
                    true;

                selectLabel = memoryLabel[0];
              } else {
                selectLabel = "Choose a Lable";
              }
            }
...
//=============facebook=======================
  ...
  addMemoryBottomSheet(BuildContext context, String text) {
...
showCursor: true,
                    ...
                    cursorColor: AppColors.primaryColor,                    onFieldSubmitted: (v) {
                      titleController.text = memoryTitleController.text;
                      selectedTitle = memoryTitleController.text;
                      focusNode.unfocus();
                      viewRefersh();
